...
def make_event_display(filename, rid, element, plane, output_dir, imgtype):
    nworkers= 4
    ...
    ws = dfc.concatenate_dataframes(ws)
    print(f"Finished loading dataframes.")
    df_load_elapsed_time = timeit.default_timer()-df_load_start_time
    print(f"Elapsed time {df_load_elapsed_time}")


        
    index = ws["trh"].index[0]

    det_keys = [ key for key in ws if key.startswith('detw') and 'TPC' in key ]

    offset=True
    
    ws["trh"]['trigger_time_cern'] = pd.to_datetime(ws["trh"]['trigger_time'])
    ws['trh']['trigger_time_cern'] = ws['trh']['trigger_time_cern'].dt.tz_convert('Europe/Zurich')
    trigger_timestamp = ws["trh"]["trigger_time"].iloc[0]
    trigger_timestamp_cern = ws["trh"]["trigger_time_cern"].iloc[0]

    trigger_types_str = "("
    for trigtype in ws["trh"]["trigger_type_bits"].iloc[0]:
    ...
    fig = plot_TPC_adc_map_mpl(df_dict=ws, det_keys=det_keys,
    ...
    while record_count < nrecords and (record_count+nskip)<len(records):
        ...
        match ws.op_env:
            ...
            case 'np04hd' | 'np04cb': 
                ...
                hd = True
            case _:
                raise RuntimeError(f'Unknown op_env: {ws.op_env}')
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=nworkers) as executor:
            ...
            future_p = {
                executor.submit(make_event_display, filename, rid, element, plane, output_dir, imgtype): (element, plane) for element, plane in myplanes
            }
