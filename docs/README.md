...
```bash
dqm_analyzer /data1/np04_hd_run022752_0000_dataflow0_datawriter_0_20230925T084543.hdf5.copied
```
In case of VD TPC additional argument `--vd` is required:
```bash
dqm_analyzer --vd /data1/np02_vd_run022748_0000_dataflow0_datawriter_0_20230925T074747.hdf5
```
Adding `--pds` will initialize checks related to the DAPHNE data and print results in a separate table:
```bash
dqm_analyzer --pds /data1/np04_hd_run022752_0000_dataflow0_datawriter_0_20230925T084543.hdf5.copied
```
And to add plotting of some stats for the collected data:
```bash
dqm_analyzer --make-plots --pds /data1/np04_hd_run022752_0000_dataflow0_datawriter_0_20230925T084543.hdf5.copied
```

### How to get PDS waveforms from hdf5 file 
dqmtools package provides a script for dumping pds waveforms for further analysis (initially used to speed up the calibration process). `dump_dps_ana_info` takes two arguments -- input directory and run number, and has several options. For the list of available options try: `dump_pds_ana_info --help`. For each input file and each channel script will produce separate file containing 2-dimensional numpy array with waveforms.

Most general usage example:
```bash
dump_pds_ana_info  /data3/ 24100
``` 
...
```bash
dump_pds_ana_info  /data3/ 24100 -nr 2 -nf 1 --cathode
``` 
this will process only 2 records in 1 file and only channles in the cathode PDS modules.

### Running dqm_plotter
`dqm_plotter` will create event display images. Use like so:
```
Usage: dqm_plotter [OPTIONS] INPUT_DATA OUTPUT_DIR
...
So, as an example ...
```
dqm_plotter --component 4 --imgtype pdf /path/to/raw_file.hdf5 temp_dir/
```
